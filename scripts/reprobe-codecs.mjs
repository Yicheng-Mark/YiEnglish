...
const TARGET_IDS = new Set(['4', '6', '7', '11', '14', '17', '20', '21', '25', '26', '32', '35', '38', '40', '41', '49', '55', '57'])

const src = fs.readFileSync(DATA_FILE, 'utf8')
const idMatches = [...src.matchAll(/id:\s*'(\d+)'/g)]
const urlMatches = [...src.matchAll(/videoUrl:\s*`\$\{OSS\}([^`]+)`/g)]
const items = urlMatches.map((m, i) => ({ id: idMatches[i]?.[1] || '?', path: m[1] })).filter((it) => TARGET_IDS.has(it.id))
