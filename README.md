...
<p align="center">
  ...
  <sub>输入 · 规则 · 记忆 · 训练 · 应用 · 反馈 —— 让每一次学习都产生积累</sub>
</p>
...
**LingoForge** 是一个把英语学习拆成「六层闭环」的独立产品：**输入 → 规则 → 记忆 → 训练 → 应用 → 反馈**。

绝大多数背单词 App 只解决「输入 + 记忆」两层，学到的东西没有出口、没有反馈、没有真正的「活英语」。LingoForge 的核心论点是：**背过的词必须回到语境里被使用、被检测、被复习，记忆才能真正留存**。因此四大功能词本（错题本 / 阅读词本 / 语料词本 / 复习计划）会把阅读、语料、练习中积累的词汇自动回流到打字训练，形成自驱的闭环。
...
- **Web Audio 机械键盘音效**：纯代码合成真实机械键盘声 + 正确/错误/完成提示音，零音频依赖
- **SM-2 间隔重复**：基于遗忘曲线在「将要遗忘前」精准召回
- **错词智能分类**：自动识别双字母遗漏、元音混淆、相邻键误触等错误类型，沉淀进错题本
- **听写模式**：隐藏单词，纯靠记忆与中文释义拼写
- **可配重复次数**：1 / 3 / 5 / 8 / 无限次，按需强化
- **跨词库搜词**：全局搜索，点击直达对应词库章节
- **四大功能词本**：错题本、阅读词本、语料词本、复习计划 —— 闭环回流的核心载体
...
### 🎬 视频语料中心
- **62 集精选语料**：TED 演讲、旅行 Vlog、生活播客、科技分享，美音英音兼备
...
### 👥 用户与账号体系
- **邮箱注册/登录**：验证码验证，JWT(access 3d) + Refresh(7d) + HttpOnly Cookie 双重鉴权
- **设备登录限制**：每账号最多 2 台设备同时在线，超出直接拒绝，支持设备管理（查看 / 踢出）
- **激活码体系**：激活码注册 + `/activate/<code>` 链接直达 + 粘贴完整链接自动提取码段
- **体验码体系**：体验码 1 小时试用、设备级限流（一设备一码）、试用条引导升级
- **找回密码**：凭激活码反查账号并重置用户名/密码
- **跨设备进度同步**：词库进度、词本、收藏状态、服务端持久化
...
| 亮点 | 说明 |
|:---|:---|
| 🔁 六层学习闭环 | 阅读/语料/错题的词汇自动回流训练，四大功能词本是闭环载体 |
...
| 🔐 设备登录限制 | 每账号 2 设备 + 激活码 / 体验码 / 找回密码完整账号体系 |
...
62 集真实英语视频语料，覆盖 **演讲 / 旅行 / 生活 / 科技** 四大类，美音英音兼备。每集提供：
...
```
                         ┌──────────────────────────────┐
   用户 ──►  www.lingoforge.fun  ──►  Nginx(:80)
                         │                    │
                         │      ┌─────────────┴──────────────┐
                         │      │                            │
                  静态 dist(/)                       /api/* 反代 :3001
