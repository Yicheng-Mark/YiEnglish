...
**Yi English** 把英语学习拆成一条**六层闭环**：**输入 → 规则 → 记忆 → 训练 → 应用 → 反馈**。
...
| 🔐 完整账号体系 | 默认 2 台设备上限（可按账号覆盖）+ 激活码 / 体验码 / 找回密码，到期双重强制下线 |
...
MAX_DEVICES_PER_USER=2           # 全局默认设备上限，0=不限（可按账号 users.max_devices 覆盖）
