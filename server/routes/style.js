...
router.patch('/name', guarded, async (req, res, next) => {
  try {
    ...
    await pool.execute('UPDATE user_style_settings SET custom_name = ? WHERE user_id = ?', [
      customName.trim(),
      req.userId,
    ])
...
router.patch('/custom-prompt', guarded, async (req, res, next) => {
  try {
    ...
    await pool.execute('UPDATE user_style_settings SET custom_prompt = ? WHERE user_id = ?', [
      value,
      req.userId,
    ])
...
router.post('/reset-personality', guarded, async (req, res, next) => {
  try {
    ...
    await pool.execute('UPDATE user_style_settings SET custom_prompt = NULL WHERE user_id = ?', [
      req.userId,
    ])
