...
function getOptionStyle(index, selected, isCorrect, correctIndex) {
  if (selected === null)
    return 'border-gray-200 dark:border-white/10 hover:border-indigo-400 dark:hover:border-indigo-500 hover:bg-indigo-50/50 dark:hover:bg-indigo-500/10'
  if (index === correctIndex)
    return 'border-emerald-500 bg-emerald-50 dark:bg-emerald-500/15 text-emerald-900 dark:text-emerald-200'
  if (index === selected && !isCorrect)
    return 'border-red-500 bg-red-50 dark:bg-red-500/15 text-red-900 dark:text-red-200'
        ...
        {options.map((opt, i) => (
          ...
          >
            ...
            {selectedOption !== null && i === correctIndex && (
              <svg
                className="w-5 h-5 text-emerald-500 ml-auto shrink-0"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
            ...
            {selectedOption !== null && i === selectedOption && !isCorrect && (
              <svg
                className="w-5 h-5 text-red-500 ml-auto shrink-0"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
