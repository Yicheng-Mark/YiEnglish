...
const WORD_BOOKS = [
  {
    ...
    colors: {
      ...
      desc: 'text-emerald-600/80 dark:text-emerald-400/70',
      status: 'text-emerald-600/70 dark:text-emerald-400/60',
      tagBg: 'bg-emerald-100 dark:bg-emerald-500/15',
  ...
  },
  {
    ...
    colors: {
      ...
      hoverBorder: 'hover:border-red-300 dark:hover:border-red-700/60',
      hoverShadow: 'hover:shadow-red-900/20',
      iconBg: 'bg-red-100 text-red-600 dark:bg-red-500/15 dark:text-red-400',
      title: 'text-red-900 dark:text-red-200',
      desc: 'text-red-600/80 dark:text-red-400/70',
      status: 'text-red-600/70 dark:text-red-400/60',
      tagBg: 'bg-red-100 dark:bg-red-500/15',
  ...
  },
  {
    ...
    colors: {
      ...
      desc: 'text-violet-600/80 dark:text-violet-400/70',
      status: 'text-violet-600/70 dark:text-violet-400/60',
      tagBg: 'bg-violet-100 dark:bg-violet-500/15',
  ...
  },
  {
    ...
    colors: {
      ...
      desc: 'text-cyan-600/80 dark:text-cyan-400/70',
      status: 'text-cyan-600/70 dark:text-cyan-400/60',
      tagBg: 'bg-cyan-100 dark:bg-cyan-500/15',
  ...
  },
  {
    ...
    colors: {
      ...
      status: 'text-amber-600/70 dark:text-amber-400/60',
      tagBg: 'bg-amber-100 dark:bg-amber-500/15',
...
function WordBooks() {
  ...
  const getCount = (id) => {
    switch (id) {
      case 'review': return counts.reviewTotal
      case 'error-book': return counts.errorBook
      case 'reading-word-book': return counts.readingWord
      case 'corpus-word-book': return counts.corpusWord
      case 'favorite-words': return counts.favoriteWord
      default: return 0
    }
  }
  ...
  const getStatus = (id) => {
    switch (id) {
      case 'review':
        if (counts.reviewDue > 0) return `${counts.reviewDue} 个单词待复习（共 ${counts.reviewTotal} 个）`
      ...
      case 'reading-word-book':
        return counts.readingWord > 0 ? `已积累 ${counts.readingWord} 个词汇` : '精选文章，在阅读中自然掌握单词'
      case 'corpus-word-book':
        return counts.corpusWord > 0 ? `已积累 ${counts.corpusWord} 个词汇` : '观看语料视频，在真实语境中掌握单词'
      case 'favorite-words':
        return counts.favoriteWord > 0 ? `已收藏 ${counts.favoriteWord} 个词汇` : '练习中收藏感兴趣的单词'
      default: return ''
    }
  }
  ...
  const getBadge = (id) => {
    switch (id) {
      case 'review': return counts.reviewDue > 0 ? counts.reviewDue : null
      case 'error-book': return counts.errorBook > 0 ? counts.errorBook : null
      default: return null
    }
  }
  ...
  return (
    <div className="min-h-screen bg-background dark:bg-transparent p-6 transition-colors duration-500 animate-page-fade-in">
      <div className="max-w-2xl mx-auto">
        <button onClick={() => navigate('/word')} className="text-content-tertiary dark:text-gray-400 hover:text-primary dark:hover:text-primary-dark mb-8 flex items-center gap-2 text-sm transition-colors px-3 py-1.5 rounded-lg hover:bg-gray-100 dark:hover:bg-white/[0.04] w-fit">
          ...
          {WORD_BOOKS.map((book) => {
            ...
            return (
              <div
                ...
                `}
              >
                <div className={`absolute top-0 left-0 w-full h-1 ${c.bar} opacity-80`} />
                ...
                <div className="flex items-start justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className={`text-lg font-bold ${c.title}`}>{book.title}</h3>
                      <p className={`text-sm ${c.desc}`}>{book.description}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    {badge !== null && (
                      <span className={`flex h-6 min-w-[1.5rem] items-center justify-center rounded-full ${c.badge} px-2 text-xs font-bold text-white shadow-sm`}>
                        {badge}
                      </span>
                    )}
                    <svg className={`w-5 h-5 ${c.desc} opacity-0 group-hover:opacity-100 transition-opacity`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div className="mt-3">
                  <div className={`inline-flex items-center rounded-lg px-2.5 py-1 text-xs font-medium ${c.tagBg} ${c.tagText}`}>
                    {book.tag}
                  </div>
                  <p className={`mt-2 text-sm ${c.status}`}>
                    {status}
                  </p>
                </div>
              </div>
