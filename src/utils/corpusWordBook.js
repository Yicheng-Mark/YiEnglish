  ...
  try {
    if (isMigrated()) {
      ...
      if (existingIndex !== -1) {
        ...
        idbPut('corpusWords', _cache[existingIndex]).catch((e) =>
          console.warn('[IDB] corpusWords put failed:', e)
        )
      } else {
...
export function removeFromCorpusWordBook(wordName) {
  try {
    ...
    removeWordFromBook('corpus', wordName).catch((e) =>
      console.warn('Sync corpus remove failed:', e)
    )
  } catch (e) {
