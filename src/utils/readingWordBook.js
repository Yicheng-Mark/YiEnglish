...
function flushIdbPuts() {
  ...
  idbBulkPut('readingWords', values).catch((e) =>
    console.warn('[IDB] readingWords bulk put failed:', e)
  )
}
...
function writeStorageNow() {
  ...
  try {
    ...
    console.error('Failed to persist reading word book:', e)
  }
  flushIdbPuts()
}
...
export async function enrichReadingWordBook() {
  // 空本早退（免得为空词本白拉一次合并索引）
  if ((getReadingWordBook().words || []).length === 0) return
  ...
  if (changed) {
    ...
    if (isMigrated()) {
      ...
      idbBulkPut('readingWords', enriched).catch((e) =>
        console.warn('[IDB] readingWords bulkPut failed:', e)
      )
    }
    replaceWordBook('reading', enriched).catch((e) =>
      console.warn('Sync enriched reading words failed:', e)
    )
  }
}
...
export function addToReadingWordBook(wordInfo) {
  try {
    ...
    enqueueServerMutation(
      wordInfo.name,
      () => addWordToBook('reading', wordInfo),
      'Sync reading add failed:'
    )
  } catch (e) {
...
export function removeFromReadingWordBook(wordName) {
  try {
    ...
    schedulePersist()
    if (isMigrated()) {
      idbDelete('readingWords', wordName).catch((e) =>
        console.warn('[IDB] readingWords delete failed:', e)
      )
    }
    enqueueServerMutation(
      wordName,
      () => removeWordFromBook('reading', wordName),
      'Sync reading remove failed:'
    )
  } catch (e) {
