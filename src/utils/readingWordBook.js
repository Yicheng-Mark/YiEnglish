  ...
  try {
    if (isMigrated()) {
      ...
      if (existingIndex !== -1) {
        ...
        idbPut('readingWords', _cache[existingIndex]).catch((e) =>
          console.warn('[IDB] readingWords put failed:', e)
        )
      } else {
        const entry = { ...wordInfo, addTime: Date.now() }
        _cache.unshift(entry)
        idbPut('readingWords', entry).catch((e) =>
          console.warn('[IDB] readingWords put failed:', e)
        )
      }
...
export function removeFromReadingWordBook(wordName) {
  try {
    ...
    removeWordFromBook('reading', wordName).catch((e) =>
      console.warn('Sync reading remove failed:', e)
    )
  } catch (e) {
