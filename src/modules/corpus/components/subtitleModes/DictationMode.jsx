...
export default function DictationMode() {
  ...
  // 与其余 7 个桌面字幕模式同款虚拟化：听写/跟读行含 textarea 与提交反馈，
  // 行高变化大，靠 measureElement 动态测量纠正。此前是唯一全量渲染的模式，
  // 300+ 句的期数切换瞬间一次性挂载数千 DOM 节点
  ...
  return (
    <div className="h-full flex flex-col p-2 md:p-3 bg-surface dark:bg-white/[0.03] border border-gray-200/70 dark:border-white/[0.06] rounded-2xl shadow-sm">
      {/* 顶部模式切换：移出滚动容器常驻（虚拟列表的滚动偏移以列表容器为基准） */}
      ...
      <div ref={scrollParentRef} {...containerProps} className="flex-1 min-h-0 overflow-y-auto">
